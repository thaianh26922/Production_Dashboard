...
const UserManagement = () => {
  ...
  useEffect(() => {
    const fetchUsers = async () => {
      try {
        const response = await axios.get('http://192.168.1.19:5000/api/users', {
          headers: {
  ...
  const handleSave = async (data) => {
    try {
      if (selectedUser) {
        await axios.put(`http://192.168.1.19:5000/api/users/${selectedUser._id}`, data, {
          headers: {
            Authorization: `Bearer ${localStorage.getItem('token')}`,
          },
        });
        ...
        const response = await axios.post('http://192.168.1.19:5000/api/users', data, {
          headers: {
            Authorization: `Bearer ${localStorage.getItem('token')}`,
          },
        });
  ...
  const handleDeleteUser = async (id) => {
    try {
      await axios.delete(`http://192.168.1.19:5000/api/users/${id}`, {
        headers: {
          Authorization: `Bearer ${localStorage.getItem('token')}`,
        },
      });
  ...
  const handleToggleLockUser = async (id) => {
    try {
      const response = await axios.put(`http://192.168.1.19:5000/api/users/${id}/lock`, {}, {
        headers: {
          Authorization: `Bearer ${localStorage.getItem('token')}`,
        },
      });
