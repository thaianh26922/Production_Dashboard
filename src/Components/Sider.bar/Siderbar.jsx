  ...
  const Sidebar = ({ isCollapsed, setIsCollapsed }) => {
    ...
    return (
      <div className={`relative dark:bg-[#305680] bg-slate-100 h-full shadow-md ${isCollapsed ? 'w-20' : 'w-60'} flex flex-col justify-between transition-all duration-300 overflow-y-auto`} style={{ maxHeight: '100vh' }}>
        ...
        <div className="flex-grow flex flex-col space-y-4 p-2  mt-4 text-gray-500">
          <nav className="flex flex-col space-y-4">
            <Link to="/dashboard" className="flex items-center text-gray-700 hover:text-black">
              ...
              {!isCollapsed && <span className="text-gray-500 hover:text-black dark:text-white">Trang chủ</span>}
            </Link>
    
            <Submenu
              title={<><FiGrid className="mr-4 text-lg dark:text-white text-gray-500" />{!isCollapsed && <span className="text-gray-500 dark:text-white hover:text-black">Nhập dữ liệu cơ bản</span>}</>}
            ...
            <Link to="/QCS/availablerate" className="flex items-center text-gray-700 hover:text-black">
              ...
              {!isCollapsed && <span className="text-gray-500 hover:text-black dark:text-white">Tỷ lệ máy chạy</span>}
            </Link>
            <Link to="/QCS/schedule" className="flex items-center text-gray-700 hover:text-black">
              ...
              {!isCollapsed && <span className="text-gray-500 hover:text-black dark:text-white">Nhiệm vụ sản xuất</span>}
            </Link>
            <Link to="/QCS/analysis" className="flex items-center text-gray-700 hover:text-black">
              ...
              {!isCollapsed && <span className="text-gray-500 hover:text-black dark:text-white">Phân tích</span>}
            </Link>
            <Link to="/QCS/reports" className="flex items-center text-gray-700  hover:text-black">
              ...
              {!isCollapsed && <span className="text-gray-500 dark:text-white hover:text-black">Báo cáo</span>}
            </Link>
            ...
            <Link to="/admin/userlist" className="flex items-center text-gray-700 hover:text-black">
              ...
              {!isCollapsed && <span className="text-gray-500 dark:text-white hover:text-black">Quản lý tài khoản</span>}
            </Link>
          )}
        ...
        <div className="p-4">
          <nav className="flex flex-col space-y-4">
            <Submenu
              title={<><FiHeadphones className="mr-4 dark:text-white text-lg text-gray-500" />{!isCollapsed && <span className="text-gray-500 dark:text-white">Hỗ trợ</span>}</>}
              items={supportItems}
            ...
            <Submenu
              title={<><FiSettings className="mr-4 text-lg dark:text-white text-gray-500" />{!isCollapsed && <span className="text-gray-500 dark:text-white">Cài Đặt</span>}</>}
              items={settingItems}
