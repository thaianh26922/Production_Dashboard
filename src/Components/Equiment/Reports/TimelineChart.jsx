...
const TimelineChart = ({ selectedDate }) => {
  ...
  const handleUpArrowClick = () => {
    if (Array.isArray(selectedDate) && selectedDate.length === 2) {
      ...
      const newStartDate = startDate.add(1, 'days'); // Cộng thêm 1 ngày vào startDate
      const newEndDate = endDate.add(1, 'days'); // Cộng thêm 1 ngày vào endDate
      
      // Update selectedDate với giá trị mới
      const newSelectedDate = [newStartDate, newEndDate];
      
      // Gọi lại fetchData với ngày mới
      fetchData(newStartDate.toDate(), newEndDate.toDate());
      
      // Bạn cũng nên cập nhật lại selectedDate state nếu cần thiết
      setConfigDate(newSelectedDate);
    }
  };
