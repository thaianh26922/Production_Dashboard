...
const WorkShiftCatalog = () => {
  ...
  const fetchWorkShifts = async () => {
    try {
      const response = await axios.get('http://192.168.127.254:5000/api/workShifts');
  ...
  const handleSave = async (values) => {
    ...
    try {
      if (selectedShift) {
        ...
        await axios.put(`http://192.168.127.254:5000/api/workShifts/${selectedShift._id}`, shiftData);
        toast.success('Cập nhật ca làm việc thành công!');
      } else {
        // Create new work shift
        console.log('Creating new work shift', shiftData.breakTimeData);
        await axios.post('http://192.168.127.254:5000/api/workShifts', shiftData);
  ...
  const handleDelete = async (id) => {
    try {
      await axios.delete(`http://192.168.127.254:5000/api/workShifts/${id}`);
